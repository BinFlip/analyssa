...
//! # Algorithm
//!
//! Based on LLVM's mem2reg `ComputeLiveInBlocks()` approach. For each variable
//! group (identified by a `u32` group ID):
//!
//! 1. **Seeding**: Start from all blocks that contain a use of the variable.
//!    These blocks are added to the worklist as seed live-in blocks.
//!
//! 2. **Backward propagation**: While the worklist is non-empty, pop a block
//!    and iterate over its predecessors. For each predecessor that does NOT
//!    define the variable, add it to the live-in set and the worklist.
//!
//! 3. **Termination**: Propagation stops at definition blocks (a definition
//!    "kills" liveness from further backward propagation).
//!
//! # Conservative Approximation
//!
//! If a block both uses and defines a variable, the use might refer to a
//! definition from outside the block. The analysis is conservative: it treats
//! use-only blocks as live-in and use+def blocks as live-in. This may place a
//! few extra phi nodes that trivial phi elimination can remove.
//!
//! # Complexity
//!
//! O(G * (B + E)) where G is the number of variable groups, B is the number of
//! blocks, and E is the number of CFG edges. Predecessors are pre-computed from
//! successors in O(E) to avoid recomputation per group.
