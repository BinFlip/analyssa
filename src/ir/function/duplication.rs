//! Block duplication, cloning, and branch target remapping for SSA functions.
//!
//! These methods support SSA transformations that need to duplicate blocks
//! (e.g., loop unrolling, jump threading, block splitting). The duplication
//! process:
//!
//! 1. Allocates fresh SSA variable IDs for all definitions in the source block
//!    via [`allocate_fresh_variables_for_block`](super::SsaFunction::allocate_fresh_variables_for_block)
//! 2. Creates corresponding [`SsaVariable`](crate::ir::variable::SsaVariable) entries
//! 3. Deep-clones the block with remapped variable references via
//!    [`clone_block_with_remap`](super::SsaFunction::clone_block_with_remap)
//! 4. Updates branch targets in the new block via
//!    [`remap_block_targets`](super::SsaFunction::remap_block_targets)
//!
//! # Variable Remapping
//!
//! When cloning a block, variables defined within the block get fresh IDs.
//! Variables that are only used (not defined) in the block retain their
//! original IDs, creating cross-block data flow edges. This is the standard
//! SSA approach: duplicated code uses the same inputs as the original but
